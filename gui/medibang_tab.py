...
class MedibangTabLoadFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(self, text="Please select the path to 'Brush2.ini' in MediBang's config folder.")
        self.label.grid(row=0, column=0, padx=10, pady=5, sticky="w")

        self.select_brush2ini_file = SelectFile(self, "ini", fg_color="transparent")
        self.select_brush2ini_file.grid(row=1, column=0, padx=10, pady=10, sticky="w")

        self.select_extract_dir = SelectDirectory(self, "Extract", fg_color="transparent")
        self.select_extract_dir.grid(row=2, column=0, padx=10, pady=10, sticky="w")
    ...
    def convert_brush(self):
        brush2ini_file_path = self.select_brush2ini_file.get_file()
        ...
        try:
            ...
            medibang_dir = os.path.dirname(brush2ini_file_path)
            load_mdp(medibang_dir, brush2ini_file_path, extract_dir)
...
class MedibangTabSaveFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.select_brush2ini_file = SelectFile(self, "ini", fg_color="transparent")
        self.select_brush2ini_file.grid(row=2, column=0, padx=10, pady=10, sticky="w")

        self.label = ctk.CTkLabel(self, text="Please select the 'brush_bitmap/' folder in MediBang's config folder.")
        self.label.grid(row=3, column=0, padx=10, pady=5, sticky="w")

        self.select_bitmap_dir = SelectDirectory(self, "Bitmap", fg_color="transparent")
        self.select_bitmap_dir.grid(row=4, column=0, padx=10, pady=10, sticky="w")

        self.convert = ConvertWidget(self, "Convert & Save", self.convert_brush, fg_color="transparent")
        self.convert.grid(row=5, column=0, padx=10, pady=20, sticky="w")
